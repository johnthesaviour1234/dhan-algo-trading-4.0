/**
 * Multi-TF Breakout WDH Strategy - Main Implementation
 * 
 * v1.0.0 - Weekly/Daily/Hourly variant (no Monthly requirement)
 * 
 * Entry: Close > ALL previous HTF highs (1H, Day, Week) for LONG
 *        Close < ALL previous HTF lows (1H, Day, Week) for SHORT
...
import { MultiTFBreakoutWDHConfig, MultiTFBreakoutWDHAnalytics, MultiTFBreakoutWDHExport, HTFLevelsWDH, DEFAULT_CONFIG } from './types';
import { backtestEngine, Trade, Metrics } from '../../lib/BacktestEngine';
import type { HTFCalculationRow } from '../../types/HTFCalculationRow';

export class MultiTFBreakoutWDHStrategy implements BaseStrategy<MultiTFBreakoutWDHConfig, MultiTFBreakoutWDHAnalytics, MultiTFBreakoutWDHExport> {
    readonly name = 'Multi-TF Breakout WDH';
    readonly version = '1.0.0';
    readonly description = 'Breakout when price exceeds HTF levels (1H/D/W - no Monthly)';
    ...
    /**
     * Detect if a new hour/day/week has started
     */
     ...
     */
    private isWithinTradingWindow(date: Date, config: MultiTFBreakoutWDHConfig): boolean {
     ...
     */
    generateSignals(ohlcData: CandlestickData[], config: MultiTFBreakoutWDHConfig = DEFAULT_CONFIG): Signal[] {
    ...
    generateSignalsWithCalculations(
        ...
        config: MultiTFBreakoutWDHConfig = DEFAULT_CONFIG
    ): {
        signals: Signal[];
        ...
        if (ohlcData.length < 100) {
            console.warn(`⚠️ [Multi-TF Breakout WDH v1.0.0] Not enough data: ${ohlcData.length} bars`);
            return {
                signals,
                calculations,
        ...
        console.log(`📊 [Multi-TF Breakout WDH v1.0.0] Processing ${ohlcData.length} bars...`);

        // Initialize HTF level tracking (NO MONTHLY)
        let levels: HTFLevelsWDH = {
        ...
        for (let i = 0; i < ohlcData.length; i++) {
            ...
            // NO MONTHLY TRACKING - allLevelsReady checks only 1H, Day, Week
            ...
            if (withinWindow && allLevelsReady) {
                ...
                if ((config.direction === 'long' || config.direction === 'both') &&
                    ...
                    if (longIsReset) {
                        ...
                        if (signals.length < 5) {
                            console.log(`🎯 [Multi-TF Breakout WDH] Signal #${signals.length + 1} at ${barDate.toISOString()}`);
                        }
            ...
            // BUILD CALCULATION ROW (with null for monthly fields)
            const calcRow: HTFCalculationRow = {
                ...
                prevMonthHigh: null,  // Not used in WDH
                prevMonthLow: null,   // Not used in WDH
                ...
                closeAboveDayHigh,
                closeAboveWeekHigh,
                closeAboveMonthHigh: false,  // Not used
                closeAboveAllHighs,

                closeBelow1HLow,
                closeBelowDayLow,
                closeBelowWeekLow,
                closeBelowMonthLow: false,  // Not used
                closeBelowAllLows,

                new1H: boundaries.new1H,
                newDay: boundaries.newDay,
                newWeek: boundaries.newWeek,
                newMonth: false,  // Not tracked
        ...
        console.log(`📊 [Multi-TF Breakout WDH] Level Readiness: 1H=${levels.prev1HReady}, Day=${levels.prevDayReady}, Week=${levels.prevWeekReady}`);
        console.log(`📊 [Multi-TF Breakout WDH] Bars in trading window: ${withinWindowCount}`);
        console.log(`📊 [Multi-TF Breakout WDH] Entry conditions met: ${conditionsMet} times`);
        ...
        console.log(`📊 [Multi-TF Breakout WDH] HOD/LOD Stats: ${totalDays} days, Max HOD=${maxHODCount}, Avg HOD=${avgHODCount.toFixed(1)}`);
        console.log(`📊 [Multi-TF Breakout WDH v1.0.0] Generated ${signals.length} signals, ${calculations.length} calculation rows`);
    ...
    runBacktest(
        ...
        console.log(`🚀 [Multi-TF Breakout WDH] Running backtest with ${ohlcData.length} bars, ₹${initialCapital} capital`);

        const { signals, calculations: allCalculations, hodLodStats } = this.generateSignalsWithCalculations(ohlcData, config);
        console.log(`📊 [Multi-TF Breakout WDH] Generated ${signals.length} signals, ${allCalculations.length} calculation rows`);
...
// Singleton instance
export const multiTFBreakoutWDHStrategy = new MultiTFBreakoutWDHStrategy();
