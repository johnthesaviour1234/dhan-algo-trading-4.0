...
export class TestingStrategy {
    ...
    start() {
        console.log('🚀 Testing strategy started');
        this.isRunning = true;
        ...
        if (MarketHours.isWithinTradingHours()) {
            ...
            console.log(`⏳ [Testing] Outside market hours (${MarketHours.getCurrentISTTimeString()} IST) - waiting...`);
        }

        // Then generate BUY signal every 1 minute (60000ms)
        this.buyInterval = setInterval(() => {
            if (this.currentPosition === null && this.isRunning) {
                ...
                if (MarketHours.isWithinTradingHours()) {
                    ...
                    console.log('⏰ [Testing] FORCE CLOSE at 2:30 PM IST');
                    this.executeSell();
                }
            }
        }, 60000); // 1 minute
    ...
    async executeBuy() {
        ...
        try {
            ...
            toast.error(`Testing: BUY failed - ${error.message}`);
    ...
    async executeSell() {
        ...
        try {
            ...
            // Create trade record
            const trade: Trade = {
                id: `testing-${Date.now()}-${this.tradeCount++}`,
            ...
            toast.success(`Testing: SELL executed at ₹${exitPrice}, P&L: ₹${pnl.toFixed(2)}`);
            console.log(`✅ SELL executed at ${exitTime.toISOString()}, price: ${exitPrice}, P&L: ${pnl}`);
            ...
            toast.error(`Testing: SELL failed - ${error.message}`);
            this.currentPosition = null;
        }
    }

    stop() {
        console.log('🛑 Testing strategy stopped');
        ...
        this.currentPosition = null;
        toast.info('Testing strategy stopped');
    }
