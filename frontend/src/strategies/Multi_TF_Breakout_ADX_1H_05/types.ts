/**
 * Multi-TF Breakout ADX 1H 0.5 Strategy - Types
 * 
 * v1.0.0 - Multi-TF Breakout (M/W/D/H) with Hourly ADX Filter + 1:0.5 R:R
 * 
 * Entry: Close > ALL previous HTF highs (1H, Day, Week, Month)
 *        AND Hourly ADX > threshold
...
export interface MultiTFBreakoutADX1H05Config extends BaseStrategyConfig {
...
export interface MultiTFBreakoutADX1H05Analytics {
    totalTrades: number;
...
export interface MultiTFBreakoutADX1H05Export {
    exportDate: string;
...
export const DEFAULT_CONFIG: MultiTFBreakoutADX1H05Config = {
    direction: 'long',
    params: {
        startHour: 9,
        startMinute: 15,
