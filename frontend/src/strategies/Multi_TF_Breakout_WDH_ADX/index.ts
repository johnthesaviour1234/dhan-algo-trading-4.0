/**
 * Multi-TF Breakout WDH ADX Strategy - Main Implementation
 * 
 ...
 * Exit: Stop Loss (prev 1H low/high) OR Target Profit (1:2 R:R) OR Market Close
 * 
 * This is a hybrid of Multi_TF_Breakout_WDH and Multi_TF_Breakout_ADX.
 */

import { CandlestickData } from 'lightweight-charts';
import { BaseStrategy, Signal, BaseTrade, BasicMetrics } from '../BaseStrategy';
import { MultiTFBreakoutWDHADXConfig, MultiTFBreakoutWDHADXAnalytics, MultiTFBreakoutWDHADXExport, HTFLevels, DEFAULT_CONFIG } from './types';
...
export class MultiTFBreakoutWDHADXStrategy implements BaseStrategy<MultiTFBreakoutWDHADXConfig, MultiTFBreakoutWDHADXAnalytics, MultiTFBreakoutWDHADXExport> {
    readonly name = 'Multi-TF Breakout WDH ADX';
    readonly version = '1.0.0';
    readonly description = 'Breakout when close > W/D/H highs (no Monthly) + Daily ADX > 25';
    ...
    /**
     * Detect if a new hour/day/week has started (no month tracking needed)
     */
     ...
     */
    private isWithinTradingWindow(date: Date, config: MultiTFBreakoutWDHADXConfig): boolean {
     ...
     */
    generateSignals(ohlcData: CandlestickData[], config: MultiTFBreakoutWDHADXConfig = DEFAULT_CONFIG): Signal[] {
    ...
    generateSignalsWithCalculations(
        ...
        config: MultiTFBreakoutWDHADXConfig = DEFAULT_CONFIG
    ): {
        signals: Signal[];
        ...
        if (ohlcData.length < 100) {
            console.warn(`⚠️ [Multi-TF Breakout WDH ADX v1.0.0] Not enough data: ${ohlcData.length} bars`);
            return {
                signals,
                calculations,
        ...
        console.log(`📊 [Multi-TF Breakout WDH ADX v1.0.0] Processing ${ohlcData.length} bars with ADX threshold ${config.params.adxThreshold} (NO Monthly)...`);

        // Initialize HTF level tracking (NO Monthly)
        ...
        for (let i = 0; i < ohlcData.length; i++) {
            ...
            // Detect timeframe boundaries FIRST (no month)
            ...
            // ============= CHECK ALL LEVELS READY (NO Monthly) =============
            ...
            if (withinWindow && allLevelsReady) {
                ...
                if ((config.direction === 'long' || config.direction === 'both') &&
                    ...
                    if (!adxConditionMet) {
                        ...
                        if (signals.length < 5) {
                            console.log(`🎯 [Multi-TF Breakout WDH ADX] Signal #${signals.length + 1} at ${barDate.toISOString()}, Daily ADX: ${currentDailyADX?.toFixed(2)}`);
                        }
            ...
            // ============= BUILD CALCULATION ROW (NO Monthly) =============
        ...
        // Debug logs
        console.log(`📊 [Multi-TF Breakout WDH ADX] Level Readiness: 1H=${levels.prev1HReady}, Day=${levels.prevDayReady}, Week=${levels.prevWeekReady}`);
        console.log(`📊 [Multi-TF Breakout WDH ADX] Bars in trading window: ${withinWindowCount}`);
        console.log(`📊 [Multi-TF Breakout WDH ADX] Entry conditions met (before ADX filter): ${conditionsMet} times`);
        console.log(`📊 [Multi-TF Breakout WDH ADX] Entries blocked by ADX: ${tradesBlockedByADX}`);
        console.log(`📊 [Multi-TF Breakout WDH ADX] Completed daily candles for ADX: ${completedDailyHighs.length}`);
        console.log(`📊 [Multi-TF Breakout WDH ADX] Final Daily ADX: ${currentDailyADX?.toFixed(2) ?? 'N/A'}`);
        ...
        console.log(`📊 [Multi-TF Breakout WDH ADX v1.0.0] Generated ${signals.length} signals, ${calculations.length} calculation rows`);
        console.log(`📊 [Multi-TF Breakout WDH ADX] Avg ADX on entry: ${avgADXOnEntry.toFixed(2)}, Days with ADX > ${config.params.adxThreshold}: ${daysWithADXAbove}/${daysWithADXAbove + daysWithADXBelow}`);
    ...
    ): MultiTFBreakoutWDHADXAnalytics {
    ...
    runBacktest(
        ...
        console.log(`🚀 [Multi-TF Breakout WDH ADX] Running backtest with ${ohlcData.length} bars, ₹${initialCapital} capital, ADX threshold: ${config.params.adxThreshold}`);

        const { signals, calculations: allCalculations, hodLodStats, adxStats } = this.generateSignalsWithCalculations(ohlcData, config);
        console.log(`📊 [Multi-TF Breakout WDH ADX] Generated ${signals.length} signals, ${allCalculations.length} calculation rows`);
        ...
        console.log(`✅ [Multi-TF Breakout WDH ADX] ${trades.length} trades generated, Time in market: ${((barsInPosition / totalMarketBars) * 100).toFixed(1)}%`);
...
// Singleton instance
export const multiTFBreakoutWDHADXStrategy = new MultiTFBreakoutWDHADXStrategy();
