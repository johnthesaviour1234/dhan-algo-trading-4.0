...
interface HTFCalculationsTableProps {
    calculations: HTFCalculationRow[];
}

export function HTFCalculationsTable({ calculations }: HTFCalculationsTableProps) {
    ...
    // Helper to render condition badge
    ...
    return (
        <div className="border-t border-gray-200 pt-6">
            <h5 className="text-gray-900 mb-4">
                HTF Calculations ({calculations.length} bars, showing last 500)
            </h5>
                                    ...
                                    return (
                                        <tr key={`${calc.timestamp}-${index}`} className={rowClass}>
                                            ...
                                            <td className="py-2 px-2 text-center">
                                                {calc.signal === 'NONE' ? (
                                                    calc.signalBlocked ? (
                                                        <span className="inline-flex px-2 py-0.5 rounded bg-yellow-100 text-yellow-700 text-xs" title={calc.blockedReason}>
                                                            ⛔
                                                        </span>
                                                    ) : (
            ...
            <div className="mt-4 flex flex-wrap gap-4 text-xs text-gray-500">
                ...
                <span>📅 New day</span>
                <span>📆 New week</span>
                <span>🗓️ New month</span>
                <span className="text-green-600 font-bold">Green = Close above level</span>
            </div>
        </div>
    );
}
